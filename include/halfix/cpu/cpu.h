...
#define XMM32(n) thread_cpu.xmm32[(n) << 2]
#define XMM16(n) thread_cpu.xmm16[(n) << 3]
#define XMM8(n) thread_cpu.xmm8[(n) << 4]
...
#define get_iopl() (thread_cpu.eflags >> 12 & 3)
...
struct cpu {
    ...
    //  - Protected mode: Should be equal to halfix.seg[CS] & 3
    ...
    // The amount to shift the TLB tag by. See docs/halfix/tlb.md for details.
...
#define thread_cpu (*thread_cpu_ptr)
extern __thread struct cpu *thread_cpu_ptr;

#define MEM32(e) *(uint32_t*)(g2h(e))
#define MEM16(e) *(uint16_t*)(g2h(e))
#define PTR_TO_PHYS(ptr) (uint32_t)(uintptr_t)(h2g(ptr))
...
#define PHYS_EIP() thread_cpu.phys_eip
#define VIRT_EIP() (thread_cpu.phys_eip + thread_cpu.eip_phys_bias)
#define LIN_EIP() (thread_cpu.phys_eip + thread_cpu.eip_phys_bias + thread_cpu.seg_base[CS])
// Modifys phys_eip accordingly
#define SET_VIRT_EIP(eip) \
    thread_cpu.phys_eip += (eip)-VIRT_EIP();
...
#define INTERNAL_CPU_LOOP_EXIT()                     \
    do {                                             \
        ...
        thread_cpu.refill_counter = thread_cpu.cycles_to_run - 1;  \
...
#define cpu_read8(linaddr, dest, shift)                                            \
    do {                                                                           \
        uint32_t addr_ = linaddr, shift_ = shift, tag = thread_cpu.tlb_tags[addr_ >> 12]; \
        if (TLB_ENTRY_INVALID8(addr_, tag, shift_)) {                              \
            if (!cpu_access_read8(addr_, tag >> shift, shift))                     \
                dest = thread_cpu.read_result;                                            \
            else                                                                   \
                EXCEPTION_HANDLER;                                                 \
        } else                                                                     \
            dest = *(uint8_t*)(thread_cpu.tlb[addr_ >> 12] + addr_);                      \
    } while (0)
#define cpu_read16(linaddr, dest, shift)                                           \
    do {                                                                           \
        ...
        if (TLB_ENTRY_INVALID16(addr_, tag, shift_)) {                             \
            ...
            dest = *(uint16_t*)(thread_cpu.tlb[addr_ >> 12] + addr_);                     \
    } while (0)
...
#define cpu_write16(linaddr, data, shift)                              \
    do {                                                               \
        ...
        if (TLB_ENTRY_INVALID16(addr_, tag, shift_)) {                 \
            ...
            *(uint16_t*)(thread_cpu.tlb[addr_ >> 12] + addr_) = data_;        \
    } while (0)
#define cpu_write32(linaddr, data, shift)                              \
    do {                                                               \
        ...
        if (TLB_ENTRY_INVALID32(addr_, tag, shift_)) {                 \
            ...
            *(uint32_t*)(thread_cpu.tlb[addr_ >> 12] + addr_) = data_;        \
    } while (0)
