...
get_thread_heap_raw(void) {
    return (heap_t*)get_tls_value(TLS_HEAP_OFFSET); //	return _memory_thread_heap;
}
...
set_thread_heap(heap_t* heap) {
    set_tls_value(TLS_HEAP_OFFSET, heap);
...
_rpmalloc_mmap_os(size_t size, size_t* offset) {
	...
	assert(size >= _memory_page_size);
    void* ptr = kmap_memory_dynamic(size + padding, PROT_RW);
	if ((ptr == (void*)-1) || !ptr) {
...
_rpmalloc_unmap_os(void* address, size_t size, size_t offset, size_t release) {
    ...
    if (release) {
        if (kunmap_memory(address, release)) {
