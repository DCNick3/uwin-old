...
// ============================================================================
static __thread uint8_t* rawp; // Physical pointer used by decoder
static __thread uint8_t prefetch[16];
static __thread int state_hash;
static __thread int seg_prefix[2] = { DS, SS };
#define rb() *rawp++
...
{
    int opcode, state_hash = thread_cpu.state_hash, initial_max_bytes = max_bytes, opcode_info;
...
top:
    ...
    case 0x66: // Operand size prefix
        if (!((state_hash ^ thread_cpu.state_hash) & STATE_CODE16))
            state_hash ^= STATE_CODE16;
        break;
    case 0x67: // Address size prefix
        if (!((state_hash ^ thread_cpu.state_hash) & STATE_ADDR16))
...
static __thread int sse_prefix = 0;
static int decode_prefix(struct decoded_instruction* i)
{
    ...
    if (prefix_set > 0) {
        ...
        seg_prefix[0] = DS;
        seg_prefix[1] = SS;
        state_hash = thread_cpu.state_hash;
    }
    sse_prefix = 0;
    return return_value;
error:
...
{
    uint8_t modrm = rb();
    int old_state_hash = thread_cpu.state_hash;
    state_hash |= STATE_CODE16;
    i->flags = parse_modrm(i, modrm, 0);
...
{
    thread_cpu.tlb_tags[lin >> 12] |= 0x44; // Mark both user and supervisor write TLBs as SMC
    int b128 = ((thread_cpu.phys_eip + length) >> 7) - (thread_cpu.phys_eip >> 7) + 1;
    for (int i = 0; i < b128; i++)
        cpu_smc_set_code(thread_cpu.phys_eip + (i << 7));
}
...
{
    state_hash = thread_cpu.state_hash;
    rawp = get_phys_ram_ptr(thread_cpu.phys_eip, 0);
    uint8_t* rawp_base = rawp;
    uintptr_t high_mark = (uintptr_t)(get_phys_ram_ptr ((thread_cpu.phys_eip & ~0xFFF) + 0xFF0, 0));
    void* original = i;
    //if(halfix.phys_eip == 0x1102b8) __asm__("int3");
        ...
        if ((uintptr_t)rawp > high_mark) {
            ...
            if (maximum_insn_length > 15 || find_instruction_length(maximum_insn_length) == -1) {
                if (instructions_translated != 0) {
                    ...
                    info->phys = thread_cpu.phys_eip;
                    info->state_hash = thread_cpu.state_hash;
                    info->flags = length;
                    info->ptr = original;
...
#define EXCEPTION_HANDLER          \
    do {                           \
        ...
        return 0;                  \
                    ...
                    uint8_t tlb_tag = thread_cpu.tlb_tags[next_page >> 12];
                    if (TLB_ENTRY_INVALID8(next_page, tlb_tag, thread_cpu.tlb_shift_read) || thread_cpu.tlb_attrs[next_page >> 12] & TLB_ATTR_NX) {
                        if (cpu_mmu_translate(next_page, thread_cpu.tlb_shift_read | 8))
                            EXCEPTION_HANDLER;
                    }
                // This is the only point at which an exception can be raised.
                for (int j = 0; j < 15; j++)
                    cpu_read8(lin_eip + j, prefetch[j], thread_cpu.tlb_shift_read);
