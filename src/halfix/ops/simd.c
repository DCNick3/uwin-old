...
///////////////////////////////////////////////////////////////////////////////
#include "halfix/softfloat/softfloat-compare.h"
...
int cpu_sse_exception(void)
{
    ...
    if ((thread_cpu.cr[4] & CR4_OSFXSR) == 0)
        EXCEPTION_UD();
    if (thread_cpu.cr[0] & CR0_EM)
        EXCEPTION_UD();
    if (thread_cpu.cr[0] & CR0_TS)
        EXCEPTION_NM();
    return 0;
}
...
void cpu_update_mxcsr(void)
{
    ...
    status.float_rounding_mode = thread_cpu.mxcsr >> 13 & 3;
    status.flush_underflow_to_zero = (thread_cpu.mxcsr >> 15) & (thread_cpu.mxcsr >> 11) & 1;
    status.float_exception_masks = thread_cpu.mxcsr >> 7 & 63;
    status.float_suppress_exception = 0;
    status.denormals_are_zeros = thread_cpu.mxcsr >> 6 & 1;
}
...
#define MM32(n) thread_fpu.mm[n].reg.r32[0]
...
static int write_back_handler(void)
{
    for (int i = 0; i < write_back_dwords; i++)
        cpu_write32(write_back_linaddr + (i * 4), temp.d128[i], thread_cpu.tlb_shift_write);
    return 0;
}
#define WRITE_BACK()                        \
...
static int get_read_ptr(uint32_t flags, struct decoded_instruction* i, int dwords, int unaligned_exception)
{
    ...
    if (linaddr & ((dwords << 2) - 1)) {
        ...
        for (int i = 0, j = 0; i < dwords; i++, j += 4)
            cpu_read32(linaddr + j, temp.d128[i], thread_cpu.tlb_shift_read);
        result_ptr = temp.d128;
        write_back_dwords = dwords;
        write_back_linaddr = linaddr;
        return 0;
    }
    uint8_t tag = thread_cpu.tlb_tags[linaddr >> 12] >> thread_cpu.tlb_shift_read;
    if (tag & 2) {
        if (cpu_mmu_translate(linaddr, thread_cpu.tlb_shift_read))
            return 1;
    }

    uint32_t* host_ptr = thread_cpu.tlb[linaddr >> 12] + linaddr;
    result_ptr = host_ptr;
    return 0;
}
static int get_write_ptr(uint32_t flags, struct decoded_instruction* i, int dwords, int unaligned_exception)
{
    ...
    uint8_t tag = thread_cpu.tlb_tags[linaddr >> 12] >> thread_cpu.tlb_shift_write;
    if (tag & 2) {
        if (cpu_mmu_translate(linaddr, thread_cpu.tlb_shift_write))
            return 1;
    }

    uint32_t* host_ptr = thread_cpu.tlb[linaddr >> 12] + linaddr;
    uint32_t phys = PTR_TO_PHYS(host_ptr);
    if (phys >= TARGET_ADDRESS_SPACE_SIZE) {
        write_back = 1;
        result_ptr = temp.d128;
        write_back_dwords = dwords;
        write_back_linaddr = linaddr;
...
static int get_mmx_write_ptr(uint32_t flags, struct decoded_instruction* i, int dwords)
{
    if (I_OP2(flags)) {
        int reg = I_RM(flags);
        result_ptr = &MM32(reg);
        thread_fpu.mm[reg].dummy = 0xFFFF;
        write_back = 0;
        return 0;
    } else
...
{
    if (I_OP2(flags)) {
        result_ptr = &thread_cpu.reg32[I_RM(flags)];
        return 0;
    } else
...
{
    thread_fpu.mm[x].dummy = 0xFFFF; // STn.exponent is set to all ones
    return &thread_fpu.mm[x].reg;
}
...
int execute_0F28_2F(struct decoded_instruction* i)
{
    ...
    case UCOMISS_XGdXEd: {
        ...
        cpu_set_eflags(eflags | (thread_cpu.eflags & ~arith_flag_mask));
        fp_exception = cpu_sse_handle_exceptions();
        break;
    }
    case UCOMISD_XGqXEq: {
...
int execute_0F50_57(struct decoded_instruction* i)
{
    ...
    case MOVMSKPS_GdXEo:
        ...
        result |= src32[2] >> 29 & 4;
        result |= src32[3] >> 28 & 8;
            thread_cpu.reg32[I_REG(flags)] = result;
        break;
    case MOVMSKPD_GdXEo:
        EX(get_sse_read_ptr(flags, i, 4, 1));
        src32 = result_ptr;
...
int execute_0FD0_D7(struct decoded_instruction* i)
{
    ...
    case PMOVMSKB_GdMEq:
        CHECK_MMX;
        EX(get_mmx_read_ptr(flags, i, 2));
            thread_cpu.reg32[I_REG(flags)] = pmovmskb(result_ptr, 8);
        break;
    case PMOVMSKB_GdXEo:
        CHECK_SSE;
        EX(get_sse_read_ptr(flags, i, 4, 1));
            thread_cpu.reg32[I_REG(flags)] = pmovmskb(result_ptr, 16);
        break;
    }
    return 0;
}
...
int execute_0FC2_C6(struct decoded_instruction* i)
{
    ...
    case PINSRW_MGqEdIb:
        CHECK_SSE;
        if (I_OP2(flags))
            op = thread_cpu.reg32[I_RM(flags)];
        else
            cpu_read16(cpu_get_linaddr(flags, i), op, thread_cpu.tlb_shift_read);
    ...
    case PEXTRW_GdMEqIb:
        ...
        dest16 = get_mmx_reg_dest(I_RM(flags));
            thread_cpu.reg32[I_REG(flags)] = dest16[imm & 3]; // Zero-extend to 32-bits
        break;
    case PEXTRW_GdXEoIb:
        CHECK_SSE;
        dest16 = get_sse_reg_dest(I_RM(flags));
            thread_cpu.reg32[I_REG(flags)] = dest16[imm & 7]; // Zero-extend to 32-bits
        break;
...
int execute_0FF1_F7(struct decoded_instruction* i)
{
    ...
    case MASKMOVQ_MEqMGq:
        CHECK_MMX;
        linaddr = thread_cpu.reg32[EDI] + thread_cpu.seg_base[I_SEG_BASE(flags)];
        src8 = get_mmx_reg_src(I_REG(flags));
        mask = get_mmx_reg_src(I_RM(flags));
        for (int i = 0; i < 8; i++) {
            if (mask[i] & 0x80)
                cpu_write8(linaddr + i, src8[i], thread_cpu.tlb_shift_write);
        }
        break;
