...
// Note that writes to address beyond halfix.memory_size can be ignored because the translation system forbids translation from MMIO pages.
...
{
    phys >>= 12;
    if (phys >= thread_cpu.smc_has_code_length)
        return 0;
    return thread_cpu.smc_has_code[phys];
}

int cpu_smc_has_code(uint32_t phys)
{
    phys >>= 7;
    if ((phys >> 5) >= thread_cpu.smc_has_code_length)
        return 0;
    return thread_cpu.smc_has_code[phys >> 5] & (1 << (phys & 31));
}
...
void cpu_smc_set_code(uint32_t phys)
{
    ...
    if ((phys >> 5) >= thread_cpu.smc_has_code_length)
        return;
    thread_cpu.smc_has_code[phys >> 5] |= 1 << (phys & 31);
}
...
{
    //printf("%08x %08x %08x %08x\n", lin, phys, halfix.smc_has_code_length, halfix.smc_has_code[phys >> 12]);
    uint32_t pageid = phys >> 12, page_info, p128, invmask, pagebase = phys & ~0xFFF;
    int start, end, quit = 0;

    if (pageid >= thread_cpu.smc_has_code_length)
        return;
    page_info = thread_cpu.smc_has_code[pageid];
    p128 = phys >> 7 & 31;
    ...
    page_info &= ~invmask;
    thread_cpu.smc_has_code[pageid] = page_info;
    if (!page_info)
...
void cpu_smc_invalidate_page(uint32_t phys){
    ...
    page_info = thread_cpu.smc_has_code[pageid], pagebase = phys & ~0xFFF, quit = 1;
    for (int i = 0; i < 31; i++) {
        uint32_t mask = 1 << i;
        if (page_info & mask) {
            uint32_t physbase = pagebase + (i << 7);
