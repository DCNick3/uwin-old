...
#define SET_C0(n) thread_fpu.status_word = (thread_fpu.status_word & ~(1 << 8)) | (n) << 8
#define SET_C1(n) thread_fpu.status_word = (thread_fpu.status_word & ~(1 << 9)) | (n) << 9
#define SET_C2(n) thread_fpu.status_word = (thread_fpu.status_word & ~(1 << 10)) | (n) << 10
#define SET_C3(n) thread_fpu.status_word = (thread_fpu.status_word & ~(1 << 14)) | (n) << 14
...
static void fpu_set_control_word(uint16_t control_word)
{
    ...
    thread_fpu.control_word = control_word;

    int rounding = thread_fpu.control_word >> FPU_ROUND_SHIFT & 3;
    switch (rounding) {
    case FPU_ROUND_NEAREST: // aka round to even
        thread_fpu.status.float_rounding_mode = float_round_nearest_even;
        break;
    case FPU_ROUND_DOWN:
        thread_fpu.status.float_rounding_mode = float_round_down;
        break;
    case FPU_ROUND_UP:
        thread_fpu.status.float_rounding_mode = float_round_up;
        break;
    case FPU_ROUND_TRUNCATE: // aka towards zero
        thread_fpu.status.float_rounding_mode = float_round_to_zero;
        break;
    }
    int precision = thread_fpu.control_word >> FPU_PRECISION_SHIFT & 3;
    switch (precision) {
    case FPU_PRECISION_24: // aka float
        thread_fpu.status.float_rounding_precision = 32;
        break;
    case FPU_PRECISION_53: // aka double
        thread_fpu.status.float_rounding_precision = 64;
        break;
    case FPU_PRECISION_64: // This is the default
        thread_fpu.status.float_rounding_precision = 80;
        break;
    }

    // Are these right?
    thread_fpu.status.float_exception_flags = 0; // clear exceptions before execution
    thread_fpu.status.float_nan_handling_mode = float_first_operand_nan;
    thread_fpu.status.flush_underflow_to_zero = 0;
    thread_fpu.status.float_suppress_exception = 0;
    thread_fpu.status.float_exception_masks = control_word & 0x3F;
    thread_fpu.status.denormals_are_zeros = 0;
}
...
{
    return thread_fpu.status_word | (thread_fpu.ftop << 11);
}
...
{
    return thread_fpu.tag_word >> (((st + thread_fpu.ftop) & 7) << 1) & 3;
}
static void fpu_set_tag(int st, int v)
{
    int shift = ((st + thread_fpu.ftop) & 7) << 1;
    thread_fpu.tag_word &= ~(3 << shift);
    thread_fpu.tag_word |= v << shift;
}

static int fpu_exception_raised(int flags)
{
    return (thread_fpu.status.float_exception_flags & ~thread_fpu.status.float_exception_masks) & flags;
}
...
{
    //__asm__("int3");
    thread_fpu.status.float_exception_flags = FPU_EXCEPTION_INVALID_OPERATION | FPU_EXCEPTION_STACK_FAULT;
...
static void fpu_commit_sw(void){
    ...
    thread_fpu.status_word |= partial_sw;
    thread_fpu.status_word &= ~bits_to_clear | partial_sw;
    bits_to_clear = 0;
    partial_sw = 0;
}

static int fpu_check_exceptions2(int commit_sw){
    int flags = thread_fpu.status.float_exception_flags;
    int unmasked_exceptions = (flags & ~thread_fpu.status.float_exception_masks) & 0x3F;
    ...
    if(commit_sw)
        thread_fpu.status_word |= flags;
    else partial_sw |= flags;

    if (unmasked_exceptions) {
        thread_fpu.status_word |= 0x8080;
...
static void fninit(void)
{
    ...
    thread_fpu.status_word = 0;
    thread_fpu.tag_word = 0xFFFF;
    thread_fpu.ftop = 0;
    thread_fpu.fpu_data_ptr = 0;
    thread_fpu.fpu_data_seg = 0;
    thread_fpu.fpu_eip = 0;
    thread_fpu.fpu_cs = 0; // Not in the docs, but assuming that it's the case
    thread_fpu.fpu_opcode = 0;
}
...
{
    return &thread_fpu.st[(thread_fpu.ftop + st) & 7];
}
...
static inline void fpu_set_st(int st, floatx80 data)
{
    ...
    thread_fpu.st[(thread_fpu.ftop + st) & 7] = data;
}
...
static int fpu_exception_masked(int excep)
{
    ...
    return (thread_fpu.control_word & excep);
}

static int fpu_push(floatx80 data)
{
    thread_fpu.ftop = (thread_fpu.ftop - 1) & 7;
    fpu_set_st(0, data);
    return 0;
}
static void fpu_pop()
...
static void fpu_update_pointers(uint32_t opcode)
{
    ...
    //    __asm__("int3");
    thread_fpu.fpu_cs = thread_cpu.seg[CS];
    thread_fpu.fpu_eip = VIRT_EIP();
    thread_fpu.fpu_opcode = opcode;
}
static void fpu_update_pointers2(uint32_t opcode, uint32_t virtaddr, uint32_t seg)
{
    ...
    thread_fpu.fpu_eip = VIRT_EIP();
    thread_fpu.fpu_opcode = opcode;
    thread_fpu.fpu_data_ptr = virtaddr;
    thread_fpu.fpu_data_seg = thread_cpu.seg[seg];
}
...
{
    uint32_t low;
    cpu_read32(linaddr, low, halfix.tlb_shift_read);
    *dest = low;
    return 0;
}
#endif
...
{
    cpu_write32(linaddr, src, thread_cpu.tlb_shift_write);
    return 0;
}
#if 0
static int read_float64(uint32_t linaddr, float64* dest)
{
    uint32_t low, hi;
    cpu_read32(linaddr, low, halfix.tlb_shift_read);
    cpu_read32(linaddr + 4, hi, halfix.tlb_shift_read);
...
{
    uint64_t x = dest;
    cpu_write32(linaddr, (uint32_t)x, thread_cpu.tlb_shift_write);
    cpu_write32(linaddr + 4, (uint32_t)(x >> 32), thread_cpu.tlb_shift_write);
    return 0;
}
...
static int fpu_check_push(void)
{
    if (fpu_check_stack_overflow(-1)) {
        ...
        if(thread_fpu.control_word & FPU_EXCEPTION_INVALID_OPERATION){
            // masked response
            fpu_push(IndefiniteNaN);
        }else thread_fpu.status_word |= 0x80; // ?
        return 1;
    }
    return 0;
}
...
static int fpu_store_f80(uint32_t linaddr, floatx80* data)
{
    ...
    int shift = thread_cpu.tlb_shift_write;
...
static int fpu_read_f80(uint32_t linaddr, floatx80* data)
{
    uint16_t exponent;
    uint32_t low, hi;
    int shift = thread_cpu.tlb_shift_read;
...
{
    int relation = floatx80_compare_internal(op1, op2, unordered, &thread_fpu.status);
...
static int fstenv(uint32_t linaddr, int code16)
{
    ...
    for (int i = 0; i < 8; i++) {
        if (fpu_get_tag(i) != FPU_TAG_EMPTY)
            fpu_set_tag(i, fpu_get_tag_from_value(&thread_fpu.st[(thread_fpu.ftop + i) & 7]));
    }
    // https://www.intel.com/content/dam/www/public/us/en/documents/manuals/64-ia-32-architectures-software-developer-vol-1-manual.pdf
    // page 203
    int x = thread_cpu.tlb_shift_write;
    //fpu_debug();
    //__asm__("int3");
    if (!code16) {
        cpu_write32(linaddr, 0xFFFF0000 | thread_fpu.control_word, x);
        cpu_write32(linaddr + 4, 0xFFFF0000 | fpu_get_status_word(), x);
        cpu_write32(linaddr + 8, 0xFFFF0000 | thread_fpu.tag_word, x);
        if (thread_cpu.cr[0] & CR0_PE) {
            cpu_write32(linaddr + 12, thread_fpu.fpu_eip, x);
            cpu_write32(linaddr + 16, thread_fpu.fpu_cs | (thread_fpu.fpu_opcode << 16), x);
            cpu_write32(linaddr + 20, thread_fpu.fpu_data_ptr, x);
            cpu_write32(linaddr + 24, 0xFFFF0000 | thread_fpu.fpu_data_seg, x);
        } else {
            uint32_t linear_fpu_eip = thread_fpu.fpu_eip + (thread_fpu.fpu_cs << 4);
            uint32_t linear_fpu_data = thread_fpu.fpu_data_ptr + (thread_fpu.fpu_data_seg << 4);
            cpu_write32(linaddr + 12, linear_fpu_eip | 0xFFFF0000, x);
            cpu_write32(linaddr + 16, (thread_fpu.fpu_opcode & 0x7FF) | (linear_fpu_eip >> 4 & 0x0FFFF000), x);
            cpu_write32(linaddr + 20, linear_fpu_data | 0xFFFF0000, x);
            cpu_write32(linaddr + 24, linear_fpu_data >> 4 & 0x0FFFF000, x);
        }
    } else {
        cpu_write16(linaddr, thread_fpu.control_word, x);
        cpu_write16(linaddr + 2, fpu_get_status_word(), x);
        cpu_write16(linaddr + 4, thread_fpu.tag_word, x);
        if (thread_cpu.cr[0] & CR0_PE) {
            cpu_write16(linaddr + 6, thread_fpu.fpu_eip, x);
            cpu_write16(linaddr + 8, thread_fpu.fpu_cs, x);
            cpu_write16(linaddr + 10, thread_fpu.fpu_data_ptr, x);
            cpu_write16(linaddr + 12, thread_fpu.fpu_data_seg, x);
        } else {
            uint32_t linear_fpu_eip = thread_fpu.fpu_eip + (thread_fpu.fpu_cs << 4);
            uint32_t linear_fpu_data = thread_fpu.fpu_data_ptr + (thread_fpu.fpu_data_seg << 4);
            cpu_write16(linaddr + 6, linear_fpu_eip, x);
            cpu_write16(linaddr + 8, (thread_fpu.fpu_opcode & 0x7FF) | (linear_fpu_eip >> 4 & 0xF000), x);
...
static int fldenv(uint32_t linaddr, int code16)
{
    uint32_t temp32;
    if (!code16) {
        cpu_read32(linaddr, temp32, thread_cpu.tlb_shift_read);
        fpu_set_control_word(temp32);

        cpu_read16(linaddr + 4, thread_fpu.status_word, thread_cpu.tlb_shift_read);
        thread_fpu.ftop = thread_fpu.status_word >> 11 & 7;
        thread_fpu.status_word &= ~(7 << 11); // Clear FTOP.

        cpu_read16(linaddr + 8, thread_fpu.tag_word, thread_cpu.tlb_shift_read);
        if (thread_cpu.cr[0] & CR0_PE) {
            cpu_read32(linaddr + 12, thread_fpu.fpu_eip, thread_cpu.tlb_shift_read);

            cpu_read32(linaddr + 16, temp32, thread_cpu.tlb_shift_read);
            thread_fpu.fpu_cs = temp32 & 0xFFFF;
            thread_fpu.fpu_opcode = temp32 >> 16 & 0x7FF;

            cpu_read32(linaddr + 20, thread_fpu.fpu_data_ptr, thread_cpu.tlb_shift_read);
            cpu_read32(linaddr + 24, thread_fpu.fpu_data_seg, thread_cpu.tlb_shift_read);
        } else {
            thread_fpu.fpu_cs = 0;
            thread_fpu.fpu_eip = 0;
            cpu_read16(linaddr + 12, thread_fpu.fpu_eip, thread_cpu.tlb_shift_read);

            cpu_read32(linaddr + 16, temp32, thread_cpu.tlb_shift_read);
            thread_fpu.fpu_opcode = temp32 & 0x7FF;
            thread_fpu.fpu_eip |= temp32 << 4 & 0xFFFF0000;

            cpu_read32(linaddr + 20, temp32, thread_cpu.tlb_shift_read);
            thread_fpu.fpu_data_ptr = temp32 & 0xFFFF;

            cpu_read32(linaddr + 24, temp32, thread_cpu.tlb_shift_read);
            thread_fpu.fpu_eip |= temp32 << 4 & 0xFFFF0000;
        }
    } else {
        cpu_read16(linaddr, temp32, thread_cpu.tlb_shift_read);
        fpu_set_control_word(temp32);

        cpu_read16(linaddr + 2, thread_fpu.status_word, thread_cpu.tlb_shift_read);
        thread_fpu.ftop = thread_fpu.status_word >> 11 & 7;
        thread_fpu.status_word &= ~(7 << 11); // Clear FTOP.

        cpu_read16(linaddr + 4, thread_fpu.tag_word, thread_cpu.tlb_shift_read);
        if (thread_cpu.cr[0] & CR0_PE) {
            cpu_read16(linaddr + 6, thread_fpu.fpu_eip, thread_cpu.tlb_shift_read);
            cpu_read16(linaddr + 8, thread_fpu.fpu_cs, thread_cpu.tlb_shift_read);
            cpu_read16(linaddr + 10, thread_fpu.fpu_data_ptr, thread_cpu.tlb_shift_read);
            cpu_read16(linaddr + 12, thread_fpu.fpu_data_seg, thread_cpu.tlb_shift_read);
        } else {
            thread_fpu.fpu_cs = 0;
            thread_fpu.fpu_eip = 0;
            cpu_read16(linaddr + 6, thread_fpu.fpu_eip, thread_cpu.tlb_shift_read);

            cpu_read16(linaddr + 8, temp32, thread_cpu.tlb_shift_read);
            thread_fpu.fpu_opcode = temp32 & 0x7FF;
            thread_fpu.fpu_eip |= temp32 << 4 & 0xF0000;

            cpu_read16(linaddr + 10, temp32, thread_cpu.tlb_shift_read);
            thread_fpu.fpu_data_ptr = temp32 & 0xFFFF;

            cpu_read32(linaddr + 12, temp32, thread_cpu.tlb_shift_read);
            thread_fpu.fpu_eip |= temp32 << 4 & 0xF0000;
        }
    }
    if (thread_fpu.status_word & ~thread_fpu.control_word & 0x3F)
        thread_fpu.status_word |= 0x8080;
    else
        thread_fpu.status_word &= ~0x8080;
    return 0;
...
int fpu_reg_op(struct decoded_instruction* i, uint32_t flags)
{
    ...
    thread_fpu.status.float_exception_flags = 0;
    ...
    case OP(0xDE, 7): {
        ...
        case 0: // FADD - Floating point add
            dst = floatx80_add(fpu_get_st(0), fpu_get_st(st_index), &thread_fpu.status);
            break;
        case 1: // FMUL - Floating point multiply
            dst = floatx80_mul(fpu_get_st(0), fpu_get_st(st_index), &thread_fpu.status);
            break;
        case 4: // FSUB - Floating point subtract
            dst = floatx80_sub(fpu_get_st(0), fpu_get_st(st_index), &thread_fpu.status);
            break;
        case 5: // FSUBR - Floating point subtract reverse
            dst = floatx80_sub(fpu_get_st(st_index), fpu_get_st(0), &thread_fpu.status);
            break;
        case 6: // FDIV - Floating point divide
            dst = floatx80_div(fpu_get_st(0), fpu_get_st(st_index), &thread_fpu.status);
            break;
        case 7: // FDIVR - Floating point divide reverse
            dst = floatx80_div(fpu_get_st(st_index), fpu_get_st(0), &thread_fpu.status);
            break;
        }
        if (!fpu_check_exceptions()) {
    ...
    case OP(0xD9, 6): { // Various complex FPU operations
        ...
        case 0: // D9 F0: F2XM1 - Compute 2^ST(0) - 1
            if (fpu_check_stack_underflow(0, 1))
                FPU_ABORT();
            res = f2xm1(fpu_get_st(0), &thread_fpu.status);
            if (!fpu_check_exceptions())
                fpu_set_st(0, res);
            break;
        case 1: // D9 F1: FYL2X - Compute ST(1) * log2(ST(0)) and then pop
            ...
            old_rounding = thread_fpu.status.float_rounding_precision;
                thread_fpu.status.float_rounding_precision = 80;
            res = fyl2x(fpu_get_st(0), fpu_get_st(1), &thread_fpu.status);
                thread_fpu.status.float_rounding_precision = old_rounding;
        ...
        case 2: // D9 F2: FPTAN - Compute tan(ST(0)) partially
            if (fpu_check_stack_underflow(0, 1))
                FPU_ABORT();
            res = fpu_get_st(0);
            if (!ftan(&res, &thread_fpu.status))
                fpu_set_st(0, res);
            break;
        case 3: // D9 F3: FPATAN - Compute tan-1(ST(0)) partially
            if (fpu_check_stack_underflow(0, 1) || fpu_check_stack_underflow(1, 1))
                FPU_ABORT();
            res = fpatan(fpu_get_st(0), fpu_get_st(1), &thread_fpu.status);
            if (!fpu_check_exceptions()) {
                fpu_pop();
                fpu_set_st(0, res);
            }
        ...
        case 4: // D9 F4: FXTRACT - Extract Exponent and mantissa of ST0
            ...
            if (fpu_check_stack_overflow(-1))
                FPU_ABORT();
            temp = fpu_get_st(0);
            res = floatx80_extract(&temp, &thread_fpu.status);
            if (!fpu_check_exceptions()) {
                fpu_set_st(0, res);
                fpu_push(temp);
            }
    ...
    case OP(0xD9, 7): {
        ...
        case 0: // FPREM - Floating point partial remainder (8087/80287 compatible)
            if (fpu_check_stack_underflow(1, 1))
                FPU_ABORT();
            flags = floatx80_remainder(fpu_get_st(0), fpu_get_st(1), &dest, &quotient, &thread_fpu.status);
            if (!fpu_check_exceptions()) {
                if (flags < 0) {
                    SET_C0(0);
                    SET_C1(0);
        ...
        case 1: // FYL2XP1 - Compute ST1 * log2(ST0 + 1) and pop
            if (fpu_check_stack_underflow(1, 1))
                FPU_ABORT();
            dest = fyl2xp1(fpu_get_st(0), fpu_get_st(1), &thread_fpu.status);
            if (!fpu_check_exceptions()) {
                fpu_pop();
                fpu_set_st(0, dest);
            }
        ...
        case 2: // FSQRT - Compute sqrt(ST0)
            dest = floatx80_sqrt(fpu_get_st(0), &thread_fpu.status);
            break;
        case 3: { // FSINCOS - Compute sin(ST0) and sin(ST1)
            ...
            if (fsincos(fpu_get_st(0), &sinres, &cosres, &thread_fpu.status) == -1) {
                SET_C2(1);
        ...
        case 4: // FRNDINT - Round ST0
            dest = floatx80_round_to_int(fpu_get_st(0), &thread_fpu.status);
            break;
        case 5: // FSCALE - Scale ST0
            if (fpu_check_stack_underflow(0, 1) || fpu_check_stack_underflow(1, 1))
                FPU_ABORT();
            dest = floatx80_scale(fpu_get_st(0), fpu_get_st(1), &thread_fpu.status);
            break;
        case 6: // FSIN - Find sine of ST0
            dest = fpu_get_st(0);
            if (fsin(&dest, &thread_fpu.status) == -1) {
                SET_C2(1);
                FPU_ABORT();
            }
            break;
        case 7: // FCOS - Find cosine of ST0
            dest = fpu_get_st(0);
            if (fcos(&dest, &thread_fpu.status) == -1) {
                SET_C2(1);
                FPU_ABORT();
            }
            break;
    ...
    case OP(0xDF, 4): // FSTSW - Store status word
        if ((opcode & 7) != AX)
            EXCEPTION_UD();
            thread_cpu.reg16[AX] = fpu_get_status_word();
        break;
...
{
    uint32_t opcode = i->imm32, linaddr = virtaddr + thread_cpu.seg_base[seg];
    floatx80 temp80;
    float64 temp64;
    float32 temp32;
    ...
    case OP(0xDF, 0): {
        ...
        case 0:
            cpu_read32(linaddr, temp32, thread_cpu.tlb_shift_read);
            temp80 = float32_to_floatx80(temp32, &thread_fpu.status);
            break;
        case 1:
            cpu_read32(linaddr, temp32, thread_cpu.tlb_shift_read);
        ...
        case 2: {
            ...
            temp80 = float64_to_floatx80(res, &thread_fpu.status);
            break;
        }
        case 3: {
            cpu_read16(linaddr, temp32, thread_cpu.tlb_shift_read);
        ...
        switch (op) {
        case 0: // FADD - Floating point add
            st0 = floatx80_add(st0, temp80, &thread_fpu.status);
            break;
        case 1: // FMUL - Floating point multiply
            st0 = floatx80_mul(st0, temp80, &thread_fpu.status);
            break;
        ...
        case 3: // FCOMP - Floating point compare and pop
            ...
            return 0;
        case 4: // FSUB - Floating point subtract
            st0 = floatx80_sub(st0, temp80, &thread_fpu.status);
            break;
        case 5: // FSUBR - Floating point subtract with reversed operands
            st0 = floatx80_sub(temp80, st0, &thread_fpu.status);
            break;
        case 6: // FDIV - Floating point divide
            st0 = floatx80_div(st0, temp80, &thread_fpu.status);
            break;
        case 7: // FDIVR - Floating point divide with reversed operands
            st0 = floatx80_div(temp80, st0, &thread_fpu.status);
            break;
        default: // FLD
            if (!fpu_check_exceptions())
                fpu_push(temp80);
    ...
    case OP(0xD9, 3): { // FSTP m64 - Store floating point register and pop
        ...
        if (fpu_check_stack_underflow(0, 0))
            FPU_ABORT();
        temp32 = floatx80_to_float32(fpu_get_st(0), &thread_fpu.status);
    ...
    case OP(0xD9, 5): { // FLDCW
        uint16_t cw;
        cpu_read16(linaddr, cw, thread_cpu.tlb_shift_read);
    ...
    case OP(0xD9, 7): // FSTCW - Store control word to memory
        cpu_write16(linaddr, thread_fpu.control_word, thread_cpu.tlb_shift_write);
        break;
    ...
    case OP(0xDF, 3): {
        ...
        case 1: { // DB
            uint32_t res;
            if (smaller_opcode & 2)
                res = floatx80_to_int32(fpu_get_st(0), &thread_fpu.status);
            else
                res = floatx80_to_int32_round_to_zero(fpu_get_st(0), &thread_fpu.status);
            if (!fpu_check_exceptions2(0))
                cpu_write32(linaddr, res, thread_cpu.tlb_shift_write);
            break;
        }
        case 2: { // DD
            uint64_t res;
            if (smaller_opcode & 2)
                res = floatx80_to_int64(fpu_get_st(0), &thread_fpu.status);
            else
                res = floatx80_to_int64_round_to_zero(fpu_get_st(0), &thread_fpu.status);
            if (!fpu_check_exceptions2(0)) {
                cpu_write32(linaddr, res, thread_cpu.tlb_shift_write);
                cpu_write32(linaddr + 4, res >> 32, thread_cpu.tlb_shift_write);
            }
            break;
        }
        case 3: { // DF
            uint16_t res;
            if (smaller_opcode & 2)
                res = floatx80_to_int16(fpu_get_st(0), &thread_fpu.status);
            else
                res = floatx80_to_int16_round_to_zero(fpu_get_st(0), &thread_fpu.status);
            if (!fpu_check_exceptions2(0))
                cpu_write16(linaddr, res, thread_cpu.tlb_shift_write);
            break;
        }
        }
    ...
    case OP(0xDD, 3): { // FSTP - Store floating point register to memory and pop
        ...
        if (fpu_check_stack_underflow(0, 0))
            FPU_ABORT();
        temp64 = floatx80_to_float64(fpu_get_st(0), &thread_fpu.status);
        if (!fpu_check_exceptions2(0)) {
    ...
    case OP(0xDD, 4): { // FRSTOR -- Load FPU context
        ...
        int offset = 14 << !is16;
        for (int i = 0; i < 8; i++) {
            if (fpu_read_f80(offset + linaddr, &thread_fpu.st[(thread_fpu.ftop + i) & 7]))
                FPU_EXCEP();
            offset += 10;
        }
        break;
    ...
    case OP(0xDD, 6): { // FSAVE - Save FPU environment to memory
        ...
        int offset = 14 << !is16;
        for (int i = 0; i < 8; i++) {
            if (fpu_store_f80(offset + linaddr, &thread_fpu.st[(thread_fpu.ftop + i) & 7]))
                FPU_EXCEP();
            offset += 10;
        }
        fninit();
    ...
    case OP(0xDD, 7): // FSTSW - Store status word to memory
        cpu_write16(linaddr, fpu_get_status_word(), thread_cpu.tlb_shift_write);
        break;
    case OP(0xDF, 4): { // FBLD - The infamous "load BCD" instruction. Loads BCD integer and converts to floatx80
        ...
        if (fpu_fwait())
            return 1;
        cpu_read32(linaddr, low, thread_cpu.tlb_shift_read);
        cpu_read32(linaddr + 4, high, thread_cpu.tlb_shift_read);
        cpu_read16(linaddr + 8, higher, thread_cpu.tlb_shift_read);
        fpu_update_pointers2(opcode, virtaddr, seg);
    ...
    case OP(0xDF, 6): { // FBSTP - Store BCD to memory
        ...
        fpu_update_pointers2(opcode, virtaddr, seg);
        floatx80 st0 = fpu_get_st(0);
        uint64_t bcd = floatx80_to_int64(st0, &thread_fpu.status);
        ...
        for(int i=0;i<9;i++) {
            ...
            result |= (bcd % 10) << 4;
            bcd /= 10; 
            cpu_write8(linaddr + i, result, thread_cpu.tlb_shift_write);
        }

            int result = bcd % 10;
            bcd /= 10;
            result |= (bcd % 10) << 4;
            cpu_write8(linaddr + 9, result | (st0.exp >> 8 & 0x80), thread_cpu.tlb_shift_write);
    ...
    case OP(0xDF, 7): { // FISTP - Store floating point register to integer and pop
        ...
        if (fpu_check_stack_underflow(0, 0))
            FPU_ABORT();
        uint64_t i64 = floatx80_to_int64(fpu_get_st(0), &thread_fpu.status);
        if (fpu_check_exceptions2(0))
            FPU_ABORT();
        cpu_write32(linaddr, (uint32_t)i64, thread_cpu.tlb_shift_write);
        cpu_write32(linaddr + 4, (uint32_t)(i64 >> 32), thread_cpu.tlb_shift_write);
        fpu_commit_sw();
        fpu_pop();
        break;
    }
...
int fpu_fxsave(uint32_t linaddr)
{
    ...
    if(cpu_access_verify(linaddr, linaddr + 288 - 1, thread_cpu.tlb_shift_write)) return 1;
    cpu_write16(linaddr + 0, thread_fpu.control_word, thread_cpu.tlb_shift_write);
    cpu_write16(linaddr + 2, fpu_get_status_word(), thread_cpu.tlb_shift_write);
    // "Abridge" tag word
    uint8_t tag = 0;
    for (int i = 0; i < 8; i++)
        if ((thread_fpu.tag_word >> (i * 2) & 3) != FPU_TAG_EMPTY)
            tag |= 1 << i;
        
    // Some fields are less than 16 or 32 bits wide, but we write them anyways. 
    // They are filled with zeros.
    cpu_write16(linaddr + 4, tag, thread_cpu.tlb_shift_write);
    cpu_write16(linaddr + 6, thread_fpu.fpu_opcode, thread_cpu.tlb_shift_write);
    cpu_write32(linaddr + 8, thread_fpu.fpu_eip, thread_cpu.tlb_shift_write);
    cpu_write32(linaddr + 12, thread_fpu.fpu_cs, thread_cpu.tlb_shift_write);
    cpu_write32(linaddr + 16, thread_fpu.fpu_data_ptr, thread_cpu.tlb_shift_write);
    cpu_write32(linaddr + 20, thread_fpu.fpu_data_seg, thread_cpu.tlb_shift_write);
    cpu_write32(linaddr + 24, thread_cpu.mxcsr, thread_cpu.tlb_shift_write);
    cpu_write32(linaddr + 28, MXCSR_MASK, thread_cpu.tlb_shift_write);
    uint32_t tempaddr = linaddr + 32;
    for (int i = 0; i < 8; i++) {
        fpu_store_f80(tempaddr, fpu_get_st_ptr(i));
        // Fill other bytes with zeros
        cpu_write16(tempaddr + 10, 0, thread_cpu.tlb_shift_write);
        cpu_write32(tempaddr + 12, 0, thread_cpu.tlb_shift_write);
        tempaddr += 16;
    }
    ...
    for (int i = 0; i < 8; i++) {
        cpu_write32(tempaddr, thread_cpu.xmm32[(i * 4)], thread_cpu.tlb_shift_write);
        cpu_write32(tempaddr + 4, thread_cpu.xmm32[(i * 4) + 1], thread_cpu.tlb_shift_write);
        cpu_write32(tempaddr + 8, thread_cpu.xmm32[(i * 4) + 2], thread_cpu.tlb_shift_write);
        cpu_write32(tempaddr + 12, thread_cpu.xmm32[(i * 4) + 3], thread_cpu.tlb_shift_write);
        tempaddr += 16;
    }
    return 0;
}
...
int fpu_fxrstor(uint32_t linaddr)
{
    ...
    if (fpu_nm_check())
        return 1;
    if(cpu_access_verify(linaddr, linaddr + 288 - 1, thread_cpu.tlb_shift_read)) return 1;

    uint32_t mxcsr;
    cpu_read32(linaddr + 24, mxcsr, thread_cpu.tlb_shift_read);
    if (mxcsr & ~MXCSR_MASK)
        EXCEPTION_GP(0);
    ...
    uint32_t temp = 0;
    cpu_read16(linaddr + 0, temp, thread_cpu.tlb_shift_read);
    fpu_set_control_word(temp);
    cpu_read16(linaddr + 2, temp, thread_cpu.tlb_shift_read);
    thread_fpu.status_word = temp;
    thread_fpu.ftop = thread_fpu.status_word >> 11 & 7;
    thread_fpu.status_word &= ~(7 << 11);

    uint8_t small_tag_word;
    cpu_read8(linaddr + 4, small_tag_word, thread_cpu.tlb_shift_read);

    cpu_read16(linaddr + 6, thread_fpu.fpu_opcode, thread_cpu.tlb_shift_read);
    thread_fpu.fpu_opcode &= 0x7FF;
    cpu_read32(linaddr + 8, thread_fpu.fpu_eip, thread_cpu.tlb_shift_read);
    cpu_read16(linaddr + 12, thread_fpu.fpu_cs, thread_cpu.tlb_shift_read); // Note: 16-bit with 4 byte gap
    cpu_read32(linaddr + 16, thread_fpu.fpu_data_ptr, thread_cpu.tlb_shift_read);
    cpu_read16(linaddr + 20, thread_fpu.fpu_data_seg, thread_cpu.tlb_shift_read); // Note: 16-bit with 4 byte gap
    uint32_t tempaddr = linaddr + 32;
    for (int i = 0; i < 8; i++) {
    ...
    tempaddr = linaddr + 160;
    for (int i = 0; i < 8; i++) {
        cpu_read32(tempaddr, thread_cpu.xmm32[(i * 4)], thread_cpu.tlb_shift_read);
        cpu_read32(tempaddr + 4, thread_cpu.xmm32[(i * 4) + 1], thread_cpu.tlb_shift_read);
        cpu_read32(tempaddr + 8, thread_cpu.xmm32[(i * 4) + 2], thread_cpu.tlb_shift_read);
        cpu_read32(tempaddr + 12, thread_cpu.xmm32[(i * 4) + 3], thread_cpu.tlb_shift_read);
        tempaddr += 16;
    }
    ...
    for(int i=0;i<8;i++){
        ...
        if(small_tag_word & (1 << i))
            tagid = fpu_get_tag_from_value(&thread_fpu.st[i]);
        tag_word |= tagid << (i * 2);
    }
    //printf("new tag word: %04x small: %02x\n", tag_word, small_tag_word);
    thread_fpu.tag_word = tag_word;
    return 0;
}
...
int fpu_fwait(void)
{
    ...
    if (thread_fpu.status_word & 0x80) {
...
void fpu_debug(void)
{
    ...
    fprintf(stderr, "FPU CS:EIP: %04x:%08x Data Pointer: %04x:%08x\n", thread_fpu.fpu_cs, thread_fpu.fpu_eip, thread_fpu.fpu_data_seg, thread_fpu.fpu_data_ptr);
    int opcode = thread_fpu.fpu_opcode >> 8 | 0xD8;
    fprintf(stderr, "Last FPU opcode: %04x [%02x %02x | %02x /%d]\n", thread_fpu.fpu_opcode, opcode, thread_fpu.fpu_opcode & 0xFF, opcode, thread_fpu.fpu_opcode >> 3 & 7);
    fprintf(stderr, "Status: %04x (top: %d) Control: %04x Tag: %04x\n", thread_fpu.status_word, thread_fpu.ftop, thread_fpu.control_word, thread_fpu.tag_word);
    for (int i = 0; i < 8; i++) {
        int real_index = (i + thread_fpu.ftop) & 7;
        floatx80 st = thread_fpu.st[real_index];
