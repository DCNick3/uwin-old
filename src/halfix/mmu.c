...
{
    for (unsigned int i = 0; i < thread_cpu.tlb_entry_count; i++) {
        uint32_t entry = thread_cpu.tlb_entry_indexes[i];
        if (entry == (uint32_t)-1)
            continue; // Don't flush entries we have already flushed
        thread_cpu.tlb[entry] = NULL;
        thread_cpu.tlb_tags[entry] = 0xFF;
        thread_cpu.tlb_entry_indexes[i] = -1;
        thread_cpu.tlb_attrs[entry] = 0xFF;
    }
    thread_cpu.tlb_entry_count = 0;
}
void cpu_mmu_tlb_flush_nonglobal(void)
{
    ...
    for (unsigned int i = 0; i < thread_cpu.tlb_entry_count; i++) {
        ...
        if (entry == (uint32_t)-1)
            continue; // Don't flush entries we have already flushed
        if ((thread_cpu.tlb_attrs[entry] & TLB_ATTR_NON_GLOBAL) == 0)
            continue;
        thread_cpu.tlb[entry] = NULL;
        thread_cpu.tlb_tags[entry] = 0xFF;
        thread_cpu.tlb_entry_indexes[i] = -1;
    ...
    thread_cpu.tlb_entry_count = thread_cpu.tlb_entry_count; // We may still have global entries.
}
...
static void cpu_set_tlb_entry(uint32_t lin, uint32_t phys, void* ptr, int user, int write, int global, int nx)
{
    ...
    if (thread_cpu.tlb_entry_count >= MAX_TLB_ENTRIES) { // Flush TLB
        cpu_mmu_tlb_flush();
#ifdef INSTRUMENT
    ...
    thread_cpu.tlb_entry_indexes[thread_cpu.tlb_entry_count++] = entry;
    thread_cpu.tlb_attrs[entry] = (nx ? TLB_ATTR_NX : 0) | (global ? 0 : TLB_ATTR_NON_GLOBAL);
    if (!ptr)
        ptr = get_phys_ram_ptr(phys, write);
    thread_cpu.tlb[entry] = (void*)(((uintptr_t)ptr) - lin);
    thread_cpu.tlb_tags[entry] = system_read | system_write | user_read | user_write;
}

uint32_t cpu_read_phys(uint32_t addr)
{
    if (addr >= TARGET_ADDRESS_SPACE_SIZE)
        abort();
    else
        return MEM32(addr);
}
...
int cpu_mmu_translate(uint32_t lin, int shift)
{
    ...
    if (!(thread_cpu.cr[0] & CR0_PG)) {
        ...
        if (!(thread_cpu.cr[4] & CR4_PAE)) {
            // https://wiki.osdev.org/Paging
            // If we do end up page faulting, #PF will push an error code to stack
            int error_code = 0;

            uint32_t page_directory_entry_addr = thread_cpu.cr[3] + (lin >> 20 & 0xFFC),
            ...
            if (page_directory_entry & 0x80 && thread_cpu.cr[4] & CR4_PSE) {
                ...
                if (combined & write_mask) { // Normally (combined & 2) == 0
                    ...
                    if (user || (thread_cpu.cr[0] & CR0_WP)) {
        ...
        page_fault:
            ...
            CPU_LOG("CR3: %08x CPL: %d\n", thread_cpu.cr[3], thread_cpu.cpl);
            CPU_LOG("EIP: %08x ESP: %08x\n", VIRT_EIP(), thread_cpu.reg32[ESP]);
            //if(halfix.cpl == 3 && !user) __asm__("int3");
            ...
            uint32_t pdp_addr = (thread_cpu.cr[3] & ~31) | (lin >> 27 & 0x18),
            ...
            // XXX yucky yucky
            uint32_t nx_mask = -1 ^ (thread_cpu.ia32_efer << 20 & 0x80000000);
            ...
            int nx_enabled = thread_cpu.ia32_efer >> 11 & 1, nx = (pde2 >> 31) & nx_enabled;
...
void cpu_mmu_tlb_invalidate(uint32_t lin)
{
    lin >>= 12;
#if 0
    if(halfix.cr[4] & CR4_PSE){
        uint32_t linbase = lin & ~1023;
        for(int i=0;i<1024;i++){
            halfix.tlb[i + linbase] = NULL;
            halfix.tlb_tags[i + linbase] = 0xFF;
        }
        return;
    }
#endif
    thread_cpu.tlb[lin] = NULL;
    thread_cpu.tlb_tags[lin] = 0xFF;
}
